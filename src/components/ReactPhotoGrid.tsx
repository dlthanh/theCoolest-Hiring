...
const imageElements: Array<HTMLImageElement> = [];
...
class ReactPhotoGrid extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    if (this.props.gridSize) {
      const container = this.props.gridSize.split("x");
  ...
  recalculateGrid = (id: string, width: number, height: number) => {
    const _imagesToShow = [...this.state.imagesToShow];

    const imageIndex = findIndex(_imagesToShow, (image) => image.id === id);
    ...
    const contenders = ["Width", "Height"];
    const winner = contenders[this.state.ladyLuck].toLowerCase() as
            | "width"
            | "height";
    const loser = first(
    ...
    if (
      ...
      const maxDimensionImage = max(_imagesToShow, (image) => image[winner]);
      ...
      const indexForBestMaxImage = _imagesToShow.reduce(function (
        result,
        image,
        index
      ) {
  ...
  getComponentStyles = (images: Array<StateImageData>) => {
    ...
    const marginSetters = ["Bottom", "Right"];
    const contenders = ["Width", "Height"];
    const winner = contenders[this.state.ladyLuck] as "Width" | "Height";
    const loser = first(without(contenders, winner));
    const marginWinner = marginSetters[this.state.ladyLuck];
    const marginLoser = first(without(marginSetters, marginWinner));

    const smallestDimensionRaw = Math.floor(
      this.state[`container${winner}` as "containerWidth"] /
            (numberOfImages - 1)
    );
    const margin = 2;
    const smallImageDimension = smallestDimensionRaw - margin;
